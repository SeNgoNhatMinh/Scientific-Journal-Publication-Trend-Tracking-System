 ...
 * @param {string} to - Recipient email address
 * @param {string} inviterName - Name of the user inviting them
 * @param {string} workspaceName - Name of the workspace
 * @param {string} joinUrl - Full join URL
 */
const sendWorkspaceInvitationEmail = async (to, inviterName, workspaceName, joinUrl) => {
  const transporter = createTransporter();

  const mailOptions = {
    from: `"SciTrend" <${envConfig.EMAIL_FROM}>`,
    to,
    subject: `You've been invited to join ${workspaceName} on SciTrend`,
    html: `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  <div class="container">
    ...
    <div class="body">
      <p>Hello,</p>
      <p><strong style="color:#e5e7eb">${inviterName}</strong> has invited you to join the workspace <strong style="color:#e5e7eb">"${workspaceName}"</strong> on SciTrend.</p>
      
      <a href="${joinUrl}" class="btn">Join Workspace</a>

      <p>If you can't click the button, copy and paste this link into your browser:</p>
      <div class="token-box">${joinUrl}</div>
      <p>If you are not expecting this invitation, you can ignore this email.</p>
    </div>
    ...
    `,
    text: `Hello,\n\n${inviterName} has invited you to join the workspace "${workspaceName}" on SciTrend.\n\nClick this link to join:\n${joinUrl}\n\nIf you did not expect this invitation, please ignore this email.\n\n— SciTrend Team`,
  };

  const info = await transporter.sendMail(mailOptions);
  console.log(`[email] Workspace invite sent to ${to} — ${info.messageId}`);
  return info;
};

module.exports = { sendPasswordResetEmail, sendWorkspaceInvitationEmail };
