...
> Dựa trên: `docs/VN/` + Schema thực tế (MongoDB)
...
> **Mục tiêu:** Dựng MongoDB schema, Auth system, Data pipeline.
...
| 1.2 | `papers` | _id, doi (unique sparse), external_ids {openalex_id, s2_id, arxiv_id, ieee_id, acm}, title (text), abstract (text), keywords[], authors[] {name, affiliation, orcid}, publication_year (idx), source_venue {name, type, issn}, data_source (openalex\|s2\|crossref\|arxiv\|ieee\|acm), fields_of_study[] (array), citation_count, open_access_url, fetched_at, updated_at |
| 1.3 | `topics` | _id, name (unique), aliases[], parent_topic_id → topics, field_of_study (idx), trend_data[] {year, paper_count, delta_pct}, growth_rate (% YoY), saturation_score (0-1), gap_score (Research Gap), related_topic_ids[], computed_at |
| 1.4 | `user_libraries` | _id, user_id → users, collection_name, description, paper_ids[] → papers[], tags[], is_default, created_at, updated_at *(compound index: user_id + collection_name unique)* |
| 1.5 | `topic_subscriptions` | _id, user_id → users, topic_id → topics, keyword (nullable, free-form), notify_frequency (instant\|daily\|weekly), last_notified_at, is_active, created_at *(compound index: user_id + topic_id unique)* |
| 1.6 | `search_history` | _id, user_id → users, query_text, filters {year_range, authors, field, source}, result_count, clicked_paper_ids[] → papers[], searched_at (TTL 90 ngày) |
| 1.7 | `data_sources` | _id, name (openalex\|s2\|crossref\|arxiv\|ieee\|acm), base_url, api_key_ref (vault ref), is_active, rate_limit_rpm, last_sync_at, last_sync_status (success\|failed\|running), sync_stats {total_fetched, new_added, updated}, config {fields_filter, year_from}, updated_at |
| 1.8 | `audit_logs` | _id, actor_id → users, actor_role (denormalized), action (PAPER_FETCH\|USER_BAN\|SOURCE_TOGGLE...), target_type, target_id, payload {before, after}, ip_address, occurred_at (TTL 2 năm) *(insert-only)* |
| 1.9 | Thiết lập index: text index (title, abstract), compound index, TTL index | — |
...
### ISSUE-003 · `[Feature]` Thu thập & Chuẩn hóa Dữ liệu (ETL Pipeline) ⭐ Priority: HIGH

**FR:** FR-001, FR-002

| Sub-task | Nội dung | Layer |
|---|---|---|
| 3.1 | ETL connector cho **OpenAlex API** → map vào schema `papers` | Backend |
| 3.2 | ETL connector cho **Semantic Scholar API** | Backend |
| 3.3 | ETL connector cho **Crossref API** | Backend |
| 3.4 | ETL connector cho **arXiv API** | Backend |
| 3.5 | ETL connector cho **IEEE Xplore** + **ACM Digital Library** | Backend |
| 3.6 | Logic deduplicate theo `doi` (unique sparse index) | Backend |
| 3.7 | Cập nhật `external_ids` khi cùng paper có từ nhiều nguồn | Backend |
...
| 3.10 | Cron job / Celery task — batch định kỳ tự động | Backend |
...
| Sub-task | Nội dung | Layer |
|---|---|---|
| 4.1 | API `GET /api/papers` — full-text search (title, abstract, keywords) | Backend |
...
| 4.5 | Ghi `search_history` mỗi lần user tìm kiếm | Backend |
...
| Sub-task | Nội dung | Layer |
|---|---|---|
| 5.1 | Component Search Bar (keyword + filter year/authors/field/source) | Frontend |
| 5.2 | Component Results Table (title, authors, year, source, citation_count) + phân trang | Frontend |
| 5.3 | Page SCR-002 — kết nối `GET /api/papers` | Frontend |
| 5.4 | Component Paper Info Card (title, abstract, DOI, open_access_url, fields_of_study) | Frontend |
...
| 6.2 | Engine tính `growth_rate`, `saturation_score` cho từng topic | Backend |
...
| 6.6 | Scheduler cập nhật `topics.trend_data`, `growth_rate`, `gap_score` định kỳ | Backend |
| 6.7 | Ghi `audit_logs` action `PAPER_FETCH` khi update topic data | Backend |
...
### ISSUE-007 · `[UI]` Dashboard & Research Gap UI ⭐ Priority: HIGH

**Màn hình:** SCR-001 (`/dashboard`), SCR-004 (`/analysis/gap`)

| Sub-task | Nội dung | Layer |
|---|---|---|
| 7.1 | Cài đặt thư viện chart (Recharts / Chart.js) | Frontend |
...
| 7.4 | Page SCR-001 — layout Dashboard, kết nối `/api/analysis/trends` | Frontend |
| 7.5 | Component Heatmap — `gap_score` của topics theo `field_of_study` | Frontend |
| 7.6 | Component Filter Tabs — chọn `fields_of_study` | Frontend |
| 7.7 | Drill-down: click heatmap → SCR-002 filter sẵn theo topic | Frontend |
| 7.8 | Page SCR-004 — kết nối `/api/analysis/gap` | Frontend |
...
| Sub-task | Nội dung | Layer |
|---|---|---|
| 8.1 | API `GET /api/libraries` — lấy danh sách collections của user | Backend |
...
| 8.7 | Component Collection List (sidebar) | Frontend |
...
| Sub-task | Nội dung | Layer |
|---|---|---|
| 9.1 | API `GET /api/admin/audit-logs` — xem `audit_logs` (filter action, actor) | Backend |
| 9.2 | API `GET /api/admin/users` — danh sách users | Backend |
| 9.3 | API `PUT /api/admin/users/{id}/role` — đổi role (ghi audit log `USER_BAN`/`USER_PROMOTE`) | Backend |
| 9.4 | API `GET /api/admin/sources` — danh sách `data_sources` + trạng thái sync | Backend |
| 9.5 | API `PUT /api/admin/sources/{id}` — bật/tắt source (`SOURCE_TOGGLE`) | Backend |
| 9.6 | Middleware bảo vệ: chỉ role `admin` được vào `/api/admin/*` | Backend |
...
| 10.4 | API `PUT /api/subscriptions/{id}` — đổi `notify_frequency` | Backend |
| 10.5 | Notification trigger: sau batch, check `topic_subscriptions` → push notify | Backend |
...
| 11.2 | Component Subscription List + notify frequency selector | Frontend |
| 11.3 | Page SCR-006 — kết nối API subscriptions | Frontend |
| 11.4 | Component Audit Log Viewer (filter action, date) | Frontend |
...
| Sub-task | Nội dung |
|---|---|
| 12.1 | IT-001: Kiểm thử xác thực (login, JWT, refresh) |
| 12.2 | IT-002: Kiểm thử phân quyền RBAC 4 role |
...
| 12.6 | ST-001: Load test search với Elasticsearch full-text |
| 12.7 | ST-002: Security test RBAC admin endpoints |

**Blocked by:** ISSUE-002, 003, 004, 006
...
|---|---|---|---|---|---|
| ISSUE-001 | MongoDB Collections (8 collections) | database | 🔴 HIGH | — | 1 |
| ISSUE-002 | Auth & User Management | feature | 🔴 HIGH | 001 | 1 |
| ISSUE-003 | ETL Data Pipeline (6 nguồn + dedup DOI) | feature | 🔴 HIGH | 001 | 1 |
| ISSUE-004 | Search API + search_history | feature | 🔴 HIGH | 003 | 2 |
| ISSUE-005 | Search & Paper Detail UI | ui | 🔴 HIGH | 004 | 2 |
| ISSUE-006 | Trend Engine + gap_score + AI | feature | 🔴 HIGH | 004 | 2 |
| ISSUE-007 | Dashboard & Visualization UI | ui | 🔴 HIGH | 006 | 3 |
| ISSUE-008 | Personal Library (paper_ids[] ref) | feature | 🔴 HIGH | 002, 005 | 3 |
| ISSUE-009 | Admin API (audit_logs, data_sources) | api | 🟡 MEDIUM | 001 | 4 |
| ISSUE-010 | Topic Subscriptions + Notify | feature | 🟡 MEDIUM | 002 | 4 |
